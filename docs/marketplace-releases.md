...
checkout, index or ref changes and does not access the network. Manifest JSON formatting may differ
from the proposal, but every value and array order must match. A `NO_RELEASE` plan is not a candidate
