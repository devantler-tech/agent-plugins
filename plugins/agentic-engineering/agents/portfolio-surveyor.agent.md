...
- **Read-only.** Use only read verbs (list/view/search/API GETs, `git log`/`git status`, file
  ...
  repository you are only reading cannot execute code through you, and you leave no write behind. Your shell access exists solely to run the source-forge CLI's read verbs — deployments are
