...
# reason. When the file changes the exemption stops matching and is UNKNOWN, so it cannot cover a
# later sync.
#
...
#
# An exemption names the exact CONTENT it was reviewed against — the file's SHA-256 — not just the
# path. A later sync replaces the file while this list stays, so anything weaker would quietly cover
# whatever arrived afterwards, including a genuine prescription that happens to read like the warning
# that was reviewed. Each line is: path, TAB, sha256 of the file, TAB, the reason; `#` starts a
# comment. When the file changes, its exemption stops matching and is UNKNOWN, so the new content has
# to be looked at rather than inheriting the old decision.
...
is_allowed() {                      # $1 = path relative to the root, $2 = that file's digest
  [ -n "$2" ] || return 1
  case $'\n'"${allowed_entries}" in
    *$'\n'"$1${tab}$2"$'\n'*) return 0 ;;
  esac
...
for surface in "${surfaces[@]}"; do
  ...
  while IFS= read -r bad; do
    ...
    if is_allowed "${rel}" "${surface_digest}"; then
      ...
      allowed_used="${allowed_used}${rel}${tab}${surface_digest}"$'\n'
      continue
    fi
...
if [ -n "${offenders}" ]; then
  ...
  echo "  One unknown field voids the whole gh request. Use the field the command actually has:" >&2
  echo "  mergedAt or mergeCommit on gh pr view and gh pr list; closedAt on gh search prs." >&2
  exit 1
fi
