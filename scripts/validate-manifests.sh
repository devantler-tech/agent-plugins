...
validate_desired_state_resources() {
  ...
  while IFS= read -r resource; do
    ...
    # Whether a maintainer-login PR may be driven is a deployment fact, not a plugin default: a
    # deployment that keeps hands-off wants the interactive-session marker to revoke driving, while one
    # that gives the engineer every PR wants it to change attribution only (#201). Pin both branches
    # and the fail-closed default together, so neither mode can be dropped or made the silent default.
    # Literal Markdown code spans, never shell expansions.
    # shellcheck disable=SC2016
    maintainer_pr_driving_contract='**Whether you may drive such a PR — update its branch, rebase or push to it, promote, merge, or close it — is the Trust gate'"'"'s maintainer-PR driving fact, `hands-off` or `attribution-only`.** Under `hands-off`, drive one only when your creation record says you made it **and** its body carries no interactive-session marker: a PR you have no record of creating is the human'"'"'s, and the marker on one you did create means the human took it over, so both are hands-off even if they look machine-authored. Under `attribution-only`, the deployment gives you every maintainer-login PR to drive under its own active-work rules; the creation record and the marker then decide only whose control channel a maintainer comment on it is, and an actionable maintainer comment on a PR you did not create stays a named blocker on that PR until it is satisfied or withdrawn. An absent, unreadable, or unrecognised value is `hands-off`: report the gap, and never infer a grant from other prose.'
    if [ -f "$plugin_dir/agents/$entrypoint.agent.md" ]; then
      case "$normalized_agent" in
        *"$maintainer_pr_driving_contract"*)
          ;;
        *)
          echo "::error::$resource: agentic-engineer must resolve maintainer-PR driving from the Trust gate with the canonical contiguous contract"
          failed=1
          resource_failed=1
          ;;
      esac
