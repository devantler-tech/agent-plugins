...
validate_desired_state_resources() {
  ...
  local portfolio_survey_classifier_argv_contract="**Invoke the classifier only in its flag form:** \`classify-default-branch-ci-runs.sh --repo OWNER/REPO --branch BRANCH --head-sha FULL_SHA\`. The helper and the read-only guard accept nothing else: a positional \`OWNER/REPO BRANCH SHA\` is denied as \`not the guarded remote-mode shape\` and the helper itself exits 2 on it, so the first invocation must already carry all three flags."
